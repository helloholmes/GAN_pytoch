...
def train(opt):
    model_G = getattr(wgan, opt.G_model)(opt)
    model_D = getattr(wgan, opt.D_model)(opt)
    ...
    lr_g = opt.lr_g
    lr_d = opt.lr_d
    optimizer_g = torch.optim.Adam(model_G.parameters(), lr_g, betas=(opt.beta1, 0.9))
    optimizer_d = torch.optim.Adam(model_D.parameters(), lr_d, betas=(opt.beta1, 0.9))
    ...
    for epoch in range(opt.max_epoch):
        ...
        for ii, (real_img, _) in tqdm(enumerate(train_dataloder)):
            ...
            if ii % opt.print_freq:
                ...
                vis.images(fix_fake_img.data.cpu().numpy()[:64], win='fixfake')
...
if __name__ == '__main__':
    opt = DefaultConfig()
    opt.parse({'G_model': 'WGAN_NetG', 'D_model': 'WGAN_NetD', 'max_epoch': 100})
